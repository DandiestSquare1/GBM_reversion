...
write.csv(dataSet, file = "data/vw_barFeatures_train_r_20161123_clean.csv",row.names=FALSE,append = FALSE)
...
#get all "SwingNULL" rows
dataSet.SwingNULL <- dataSet[ sample( which(dataSet$ClassChar=="SwingNULL"), round(.05*length(which(dataSet$ClassChar=="SwingNULL"))),replace = TRUE), ]
...
dataSet2 <- rbind(dataSet[ which(dataSet$ClassChar!="SwingNULL"), ],dataSet.SwingNULL)
...
rm(dataSet.SwingHi, dataSet.SwingLo, dataSet2,dataSet.SwingNULL)
gc()
...
write.csv(dataSet, file = "data/vw_barFeatures_train_r_20161123_subsampled_SwingNULL.csv",row.names=FALSE,append = FALSE)
...
subsampled_SwingNULL <- read.csv("data/vw_barFeatures_train_r_20161123_subsampled_SwingNULL.csv", header = TRUE)
...
dsTrainingTest <- dataSet
dsValidation <- dataSet
...
gbm1 <- gbm(dsTrainingTest[,classColumn]~ ., data=dsTrainingTest[,x_col_start_pos:x_col_end_pos],n.cores=2,verbose=TRUE
            ,n.trees=150,shrinkage=0.005,interaction.depth=3,n.minobsinnode = 10,bag.fraction = 0.8,train.fraction = 0.6
            ,distribution="multinomial"
        )
gbm1.fit <- gbm.fit(x=as.matrix(dsTrainingTest[,x_col_start_pos:x_col_end_pos]), y=as.matrix(dsTrainingTest[,classColumn]),verbose=TRUE
            ,response.name = "SwingRecordType"
            ,n.trees=150 ,shrinkage=0.004 ,interaction.depth=2 ,n.minobsinnode=10 ,bag.fraction=0.6 ,train.fraction=0.6
            ,distribution="multinomial"
        )
...
model$n.trees#100
model$shrinkage#0.001
model$interaction.depth#1
model$n.minobsinnode#10
model$bag.fraction#.5
...
x <- dsValidationBig[,x_col_start_pos:x_col_end_pos];  y <- dsValidationBig[,classColumn];
nrow(dsValidationBig)
nrow(x)

#x <- xBkup.bal; y <- yBkup.bal;
pred <- as.data.frame(predict(gbm1.fit, x, n.trees = 150,type = 'response'));
...
## save this model
save(gbm1.fit, file = "model_GBM_20161214_8.rda")
...
ruleExec <- extractRules(treeList,X,ntree = 500, maxdepth = 1000) 
# measure rules
...
#extractedRules[,5:6]
nrow(extractedRules)
table(extractedRules[,6]) ;table(extractedRules[,5:6]) ;
write.csv(extractedRules, file = "readableRules_GBM_20161214_8.csv",row.names=TRUE)

#  class(readableLearner)
#  nrow(readableLearner)
#  print(readableLearner)

#appy learner to data
...
freqPattern <- getFreqPattern(ruleMetric,minsup = 0.000001, minconf = 0.00005, minlen = 1, maxlen = 31)
ruleMetric
freqPattern
...
gbm1 <- gbm.fit(x=as.matrix(dsTrainingTest[,x_col_start_pos:x_col_end_pos]), y=as.matrix(dsTrainingTest[,classColumn]),verbose=T
                ,response.name = "SwingRecordType" 
                ,n.trees=150,shrinkage=0.4,interaction.depth=1,n.minobsinnode = 10,bag.fraction = .8,train.fraction = .6
                ,distribution="multinomial"
              )
gbm1 <- gbm(dsTrainingTest[,classColumn]~ ., data=dsTrainingTest[,x_col_start_pos:x_col_end_pos],n.cores=2,verbose=TRUE
            ,n.trees=150,shrinkage=0.4,interaction.depth=1,n.minobsinnode = 10,bag.fraction = .8,train.fraction = .6
...
pred <- as.data.frame(predict(gbm1, xTest, n.trees = best.iter,type = 'response'));
names(pred) <- c(levels(yTest))
...
X <- dsValidation[,x_col_start_pos:x_col_end_pos]; target <- dsValidation[,classColumn];
X <- dsTrainingTest[,x_col_start_pos:x_col_end_pos]; target <- dsTrainingTest[,classColumn];
...
lists = list()
lists[['gbm']] <- GBM2List(gbm1.fit,X)
  treeList <- lists[['gbm']]

  
  
  
  ruleExecUnique <- unique(ruleExec0) # unique rules
  cat( paste("There are ", length(ruleExecUnique), " unique conditions. \n",sep="") )
  ...
  ix <- which(as.numeric(ruleMetric[,"freq"])>0.0000001 & as.numeric(ruleMetric[,"err"])< 0.999)
  ruleMetric <- ruleMetric[ix,]
  
  ruleMetric <- selectRuleRRF(ruleMetric,X,target) # rule selection
  learner <- buildLearner(ruleMetric,X,target,minFreq = 0.00000001) #build the simplified tree ensemble learner
...
readableLearner
write.csv(readableLearner, file = "readableRules_GBM_20161207_2.csv",row.names=TRUE)
ruleMetric
freqPattern

nrow(X);ncol(X)
nrow(dsValidation[,classColumn]);ncol(dsValidation[,classColumn])
...
treeList <- GBM2List(gbm1.fit,dsValidation[,x_col_start_pos:x_col_end_pos])  
# R-executable conditions
exec <- extractRules(treeList,dsValidation[,x_col_start_pos:x_col_end_pos],ntree = 150, maxdepth = 100)
exec[1:10,]
# get rule metrics
ruleMetric <- getRuleMetric(exec,dsValidation[,x_col_start_pos:x_col_end_pos],dsValidation[,classColumn]) 
ruleMetric[1:10,]
ruleMetric <- pruneRule(ruleMetric,X,target)
ruleMetric[1:2,]
ruleMetric <- selectRuleRRF(ruleMetric,dsValidation[,x_col_start_pos:x_col_end_pos],dsValidation[,classColumn])
ruleMetric
learner <- buildLearner(ruleMetric,dsValidation[,x_col_start_pos:x_col_end_pos],dsValidation[,classColumn])
learner
readableRules <- presentRules(ruleMetric,colnames(dsValidation[,x_col_start_pos:x_col_end_pos]))
readableRules[,]

freqPattern <- getFreqPattern(ruleMetric)
freqPattern[which(as.numeric(freqPattern[,"len"])>=1),][,]
...
treeList <- GBM2List(gbm1.fit,X)
ruleExec = extractRules(treeList,X,ntree = 150, maxdepth = 500)
...
readableLearner <- presentRules(learner,colnames(dsValidation[,-1])) 
err <- 1-sum(pred==target)/length(pred);
err
readableLearner
readableLearner <- colnames(dsValidation[,-1])
readableLearner

treeList <- GBM2List(gbmFit,X)
ruleExec  <-  extractRules(treeList,X,ntree = 150,maxdepth = 150)
ruleExec
ruleExec <- unique(ruleExec)
#ruleExec <- ruleExec[1:min(2000,length(ruleExec)),,drop=FALSE]
ruleMetric <- getRuleMetric(ruleExec,X,target)
...
#err <- 1-sum(pred==target)/length(pred);
readableLearner
write.csv(readableLearner, file = "readableRules_GBM_20161207_1.csv",row.names=TRUE)
...
#Make rules more readable:
readableRules <- presentRules(learner,colnames(dsValidation[,-1]))  

readableRules <- presentRules(ruleMetric,colnames(dsValidation[,-1]))  

readableRules <- presentRules(ruleMetric_pruned,colnames(dsValidation[,-1]))  
readableRules.df <- as.data.frame(readableRules)
readableRules.df[,1:3] <- sapply(readableRules.df[,1:3],as.character)
readableRules.df[,1:3] <- sapply(readableRules.df[,1:3],as.numeric)
table(sapply(readableRules.df, class))
