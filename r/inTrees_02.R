...
dataSet.SwingNULL <- dataSet[ sample( which(dataSet$ClassChar=="SwingNULL"), round(.023*length(which(dataSet$ClassChar=="SwingNULL"))),replace = TRUE), ]
...
write.csv(dataSet, file = "data/vw_barFeatures_train_r_20161123_subsampled_null.csv",row.names=FALSE,append = FALSE)

dataSet[1:12,401:412]
...
dsTrainingTest <- read.csv("data/vw_barFeatures_train_r_20161123_subsampled_null.csv", header = TRUE)
dsValidation <- read.csv("data/vw_barFeatures_train_r_20161123_clean.csv", header = TRUE)

#columns
ncol(dsTrainingTest);ncol(dsValidation)
#rows
...
gbm1 <- gbm(dsTrainingTest[,classColumn]~ ., data=dsTrainingTest[,x_col_start_pos:x_col_end_pos],n.cores=2,verbose=TRUE
            #,n.trees=50, interaction.depth=10, n.minobsinnode = 2, shrinkage=0.61#,train.fraction = 0.15
...
table(dsValidation[,classColumn])
round(prop.table(table(dsValidation[,classColumn])) * 100, digits = 2)

nrow(dsValidation)
nrow(x)
#x <- xBkup.bal; y <- yBkup.bal;
pred <- as.data.frame(predict(gbm1, x, n.trees = 100,type = 'response'));
